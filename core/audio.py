...
class VoiceCaddyAudioEngine:
    ...
    def transcribe(self, audio_path: str | Path, engine_mode: str = "cloud", api_key: Optional[str] = None, language: str = "it") -> Tuple[str, dict]:
        """
        Unified transcription method dispatching to Cloud or Local engine.
        """
    ...
    def transcribe_multiple(self, audio_paths: List[str | Path], engine_mode: str = "cloud", api_key: Optional[str] = None, language: str = "it") -> Tuple[str, List[dict]]:
        full_transcripts = []
        all_metadata = []

        for idx, path in enumerate(audio_paths, 1):
            text, meta = self.transcribe(path, engine_mode=engine_mode, api_key=api_key, language=language)
